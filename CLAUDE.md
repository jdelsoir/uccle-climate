...
- **DateNav = 3D calendar-icon** (CSS, no images): red month header (`bg-cal-header`, white text 4.5:1) + binding tabs + big day number, on the right; ◀▶ arrows below it; weekday + full date + **Today** button (jumps to `max`) on the left. The card opens the **native** date picker (month/year/day); props unchanged. Responsive 375/768/1024, no horizontal overflow.
