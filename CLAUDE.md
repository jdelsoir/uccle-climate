...
- **Today tab = one shared layout across Day/Month/Year.** `Today.tsx` owns the cursor for all three modes and renders the shared header: section title · **Today** button · **◀▶** (step the active unit: day/month-wrap/year) · underline Day/Month/Year tabs (`role=radiogroup`). Each view is controlled. Shared presentational components in `src/components/`: **`CalendarTile`** (CSS 3D calendar card — red `bg-cal-header` header + binding tabs + big body; Day tile opens the native date picker on click; Month=`JUNE/2026`, Year=`YEAR/2026`), **`RangeBar`** (record-low→high track, `role="img"` + text summary, markers tick/dot/diamond; labels staggered across two rows so close values don't overlap), **`StatCard`** (2×2 grid; optional `onClick`→jump), **`WarmingStrip`** (then→now + Δ badge). `DateNav` was removed (superseded). Day hero: TODAY'S HIGH + NOW (today, live) / HIGH + LOW (past) + rank badge + record-broken banner + provisional marker; today is live-owned (never falls back to a series entry). Month/Year: tile + mean + rank + RangeBar + 2×2 cards (Average · vs-average Δ · record warm/cold) + WarmingStrip. Responsive 375/768/1280, no overflow.
