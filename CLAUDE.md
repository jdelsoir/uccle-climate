...
12. **Month page Phase A — "the month in detail"** — new per-year `daily/YYYY.json` layer (provisional-aware record flags) → `MonthHeatmap` calendar (click→Day) + `monthSummary` day-mix/records line + `NotableDays` warmest/coldest toggle + hero+heatmap share + year-aware Month cursor & `?m=` deep link. Spec/plan in `docs/superpowers/`. (Phase B parked: per-month warming rate, trend line, tmax/tmin split, monthly counters, then-now parity.)
...
- **Month Phase B (parked):** per-month warming rate + trend line on the scatter, mean-high/mean-low split in the hero, monthly counters vs normal (summer/frost/tropical/ice days — needs per-month pipeline computation), then-now window parity with Day.
