...
- **Month highs/lows & counts (Phase B2).** Pipeline: `monthly_means` also averages tmax/tmin → `meanMax`/`meanMin` per year in `month_data.series`; new `monthly_counter_normals` → per-month 1991–2020 mean threshold-day counts in `month_data.counterNormals`. App: **hero high/low subline** (`↑ avg high · ↓ avg low` from `cur.meanMax`/`meanMin`, inside `#month-capture`). **`MonthCounters` card** ("This {month} by the numbers") — viewed-month counts via `monthDetail.monthCounters(mDays)` (consolidated `daily/YYYY.json` days; live-today excluded) vs `counterNormals`, showing a counter row only when `normal≥0.5 || count>0` (in-season or occurred); lucide icons; header "so far" when the month is incomplete. **Thresholds (SU≥25, hot30≥30, TR≥20, FD<0, ID<0) are duplicated across `derive.threshold_counters`, `derive.monthly_counter_normals`, and `lib/monthDetail.monthCounters`** — a change must touch all three.
...
13. **Month Phase B1 — "how this month is changing"** — app-side climate signal: `lib/trend.ts` (OLS) → full-record **Warming** °/decade StatCard + opt-in OLS **trend line** on `PeriodScatter` (fits shown period) + **then-now parity** with Day (viewed-year-relative windows via `windowMean`). No pipeline change. 
14. **Month Phase B2 — "highs, lows & counts"** — pipeline `meanMax`/`meanMin` (monthly mean high/low) + `monthly_counter_normals` (per-month 1991–2020 threshold-day averages) → hero **high/low subline** + **MonthCounters** card (viewed-month summer/hot/tropical/frost/ice counts vs normal, in-season only). Completes the Month tab; no Month phases remain.
