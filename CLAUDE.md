...
- **One shared layout across Day/Month/Year.** `Today.tsx` owns the cursor for all three modes + the shared header: heading · **Today** button · **◀▶** (steps the active unit) · underline Day/Month/Year tabs (`role=radiogroup`); each view is controlled. **Square corners throughout** (no `rounded-*`); responsive 375/768/1280, light+dark. `Today.tsx` also reads a **`?d=YYYY-MM-DD` deep link** (`useSearchParams`, regex-validated + clamped to [1833-01-01, today]) to open Day at a date (remounts per nav, so the param-less Nav link still resets to today).
